...
import { DialogConfig, DialogLayout, DialogOpenModel } from "./dialog.models";
...
export class DialogService {
  ...
  open<T, D = any, R = any>(
    ...
    const dialogRef: DialogRef<R> = this._createDialogRef<R>(
      overlayRef,
      config
    );
  ...
  protected _createOverlay(overlayConfig?: DialogLayout): OverlayRef {
    let positionStrategy = this._getPositionStrategy(overlayConfig);
    ...
    if (
      ...
      if (overlayConfig?.customOpenAnimation) {
        ...
        // applying default open animation
        keyframes = [
          {
  ...
  protected _getPositionStrategy(
    overlayConfig?: DialogLayout
  ): PositionStrategy {
    let positionStrategy;
    if (overlayConfig?.attachTarget) {
      // ? dialog should only be global positioned ?
  ...
  protected _createDialogRef<R>(
    ...
    const dialogRef = new DialogRef<R>(
      overlayRef,
      overlayConfig?.backgroundClosable
    );
