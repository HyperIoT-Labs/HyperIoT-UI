...
export class PacketFormComponent
  ...
  typeOptions: Option[] = Object.keys(HPacket.TypeEnum).map((k) => ({
    label: k,
    value: k,
  }));

  serializationOptions: Option[] = Object.keys(
    HPacket.SerializationEnum
  ).map((k) => ({ label: k, value: k }));

  formatOptions: Option[] = Object.keys(HPacket.FormatEnum).map((k) => ({
    label: k,
    value: k,
    disabled: k === HPacket.FormatEnum.TEXT?true:false
  }));

  trafficPlanOptions: Option[] = Object.keys(
    HPacket.TrafficPlanEnum
  ).map((k) => ({ label: k, value: k }));
  ...
  hpacketTypeChange(event) {
    ...
    if (this.entity.type === "OUTPUT") {
      ...
      if (this.form.get("hpacket-format").value === HPacket.FormatEnum.TEXT) {
        //resetting the value
        this.form.get("hpacket-format").setValue(HPacket.FormatEnum.JSON);
      }
    }
    this.defineTopics(this.entity);
  }
  ...
  private savePacket(successCallback?, errorCallback?) {
    ...
    p.unixTimestampFormatSeconds = /true/i.test(
      this.form.value["hpacket-unixTimestampFormatSeconds"]
    );
    ...
    if (p.id) {
      ...
      p.device = {
        ...
        entityVersion: this.currentDevice.entityVersion,
      };
  ...
  private deletePacket(successCallback?, errorCallback?) {
    this.loadingStatus = LoadingStatusEnum.Saving;
    this.hPacketService.deleteHPacket(this.entity.id).subscribe(
      (res) => {
        this.loadingStatus = LoadingStatusEnum.Ready;
      ...
      switch (err.error.type) {
        case "it.acsoftware.hyperiot.base.exception.HyperIoTDuplicateEntityException": {
          this.validationError = [
            {
              message: $localize`:@@HYT_unavailable_packet_name:Unavailable packet name`,
              field: "hpacket-name",
              invalidValue: "",
            },
          ];
