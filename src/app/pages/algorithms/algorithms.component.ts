...
export class AlgorithmsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.algorithmService.nextAlgorithms.state === "delete-loading") {
      this.algorithmsInLoading(
        this.algorithmService.nextAlgorithms.algorithmToDelete
      );
    }
  ...
  search(value: string) {
    if (value) {
      if (value.split("*").length > 18) {
        ...
        this.algorithmsFiltered = this.algorithms.filter((el) =>
          el.name.match(reg)
        );
  ...
  openDeleteDialog(emit: CardEmittedValue) {
    let alg = emit.data as Algorithm;
    const dialogRef = this.dialog.open(DeleteConfirmDialogComponent, {
      data: {
        title: "Are you sure you want to delete the algorithm?",
  ...
  algorithmsInLoading(algorithmId: number) {
    ...
    const algorithmToDelete =
      this.algorithmService.nextAlgorithms.algorithmList.find(
        (x) => x.id === algorithmId
      );
  ...
  updateAlgorithms(algorithmList: Algorithm[]) {
    ...
    this.pageStatus =
      this.algorithms.length !== 0 ? PageStatus.Standard : PageStatus.New;
    this.algorithmsFiltered = [...this.algorithms];
    /* Default Sort */
  }
