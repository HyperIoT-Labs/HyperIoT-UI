...
import { AlgorithmsService, Algorithm, AlgorithmOutputField } from '@hyperiot/core';
...
export class OutputFieldsFormComponent extends AlgorithmFormEntity implements OnInit, AfterViewInit {
  ...
  fieldMultiplicityOptions: Option[] = Object.keys(AlgorithmOutputField.MultiplicityEnum)
    .map((k) => ({ label: k, value: k }));

  fieldTypeOptions: Option[] = Object.keys(AlgorithmOutputField.TypeEnum)
    .map((k) => ({ label: k, value: k }));

  currentOutputField: AlgorithmOutputField;
  ...
  addField(e) {
    this.currentOutputField = this.newEntity() as AlgorithmOutputField;
  ...
  private createFieldsTree(nodes: any) {
    ...
    return nodes.map((of: AlgorithmOutputField) => ({
      data: of,
      root: false,
      name: of.name,
      lom: of.multiplicity,
  ...
  private loadFormData() {
    ...
    this.form.get('outputField-type').setValue(this.currentOutputField.type);
    // reset form
    this.resetForm();
  }
  ...
  private openDelete(fieldId: number) {
    ...
    dialogRef.onClosed.subscribe((result) => {
      if (result === 'delete') {
        ...
        this.algorithmsService.deleteOutputField(this.entity.id, fieldId).subscribe(
          res => {
  ...
  private saveAlgorithm(successCallback, errorCallback) {
    ...
    this.currentOutputField.type = this.form.get('outputField-type').value;
    let saveObservable: Observable<any>;
    if (this.currentOutputField.id > 0) {
      saveObservable = this.algorithmsService
        .updateOutputField(this.entity.id, this.currentOutputField);
    } else {
      saveObservable = this.algorithmsService
        .addOutputField(this.entity.id, this.currentOutputField);
    }
