...
import { AlarmsService, HytAlarm, Logger, LoggerService, RealtimeDataService } from 'core';
...
export class LiveAlarmsEffects {
    ...
    load$ = createEffect(() => this.actions$.pipe(
        ...
        switchMap(
            ([actions, projects]) => this.alarmsService.findAlarmStatusByProjectId(Object.keys(projects).map(id => +id))
        ),
        map(res => {
            let allAlarms = [];
        ...
        map(res => {
            return LiveAlarmsActions.loadLiveAlarmsSuccess({ liveAlarms: res });
        }),
        catchError((error) => {
        ...
        this.realtimeDataService.eventStream.pipe(
            ...
            concatLatestFrom(
                (p) => {
                    ...
                    return this.store.select(HDeviceSelectors.selectHDevcicesByRuleId({ ruleId: event.ruleId }))
                }
            ),
        ).subscribe(([p, devices]) => {
            const packet = p.data;
